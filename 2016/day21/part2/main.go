...
var (
	inverseOps []operation.Operation
)
...
func init() {
	...
	for scanner.Scan() {
		...
		if match != nil {
			op = &operation.InverseRotateOnLetterOp{X: rune(match[0][1][0])}
		}
		...
		inverseOps = append(inverseOps, op)
	}
	for i := 0; i < len(inverseOps)/2; i++ {
		inverseOps[i], inverseOps[len(inverseOps)-i-1] = inverseOps[len(inverseOps)-i-1], inverseOps[i]
	}
}

func main() {
	unscrambledPW := []rune(scrambledPW)
	for _, op := range inverseOps {
		unscrambledPW = op.Execute(unscrambledPW)
	}
	log.Printf("unscrambling %s gives you %s\n", scrambledPW, string(unscrambledPW))
}
