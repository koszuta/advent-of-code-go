...
func main() {
	...
	doMove := func(move int, shape Shape) int {
		rock := Rock{
			shape:  shape % NShapes,
			origin: Vec2{x: 2, y: top + 4},
		}
		// PrintRocks(rock)
	...
	// Go until you find a pattern
	var move int
	var shape Shape
	for move, shape = 0, HBar; shape < 100000; shape++ {
		move = doMove(move, shape)
	}

	// Drop the remaining rocks to reach 1 trillion
	for ; shape < 1_000_000_000_000; shape++ {
		move = doMove(move, shape)
	}
	log.Println("highest rock:", top+1)
}
