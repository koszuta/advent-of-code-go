...
func main() {
	...
	for i, seg1 := range segments1 {
		for j, seg2 := range segments2 {
			...
			segmentsIntersect, intersection := getIntersection(seg1, seg2)
	...
	for _, intersection := range intersections {
		dist := manhattan(centralPort.start, intersection.intersect)
		if minManhattan < 0 || dist < minManhattan {
			minManhattan = dist
		}
	}
	...
	minSteps := -1
	for _, intersection := range intersections {
		if minSteps < 0 || intersection.stepsToIntersect < minSteps {
			minSteps = intersection.stepsToIntersect
		}
	}
...
func getIntersection(seg1, seg2 segment) (bool, intersection) {
	var segmentsIntersect bool
	intersection := intersection{-1, point{}}

	if seg1.start.x == seg1.end.x {
		segmentsIntersect = ((seg1.start.y <= seg2.start.y && seg1.end.y >= seg2.start.y) || (seg1.end.y <= seg2.start.y && seg1.start.y >= seg2.start.y)) &&
			((seg2.start.x <= seg1.start.x && seg2.end.x >= seg1.start.x) || (seg2.end.x <= seg1.start.x && seg2.start.x >= seg1.start.x))
		if segmentsIntersect {
			intersection.intersect.x = seg1.start.x
			intersection.intersect.y = seg2.start.y
			intersection.stepsToIntersect = (seg1.stepsToEnd - (seg1.end.y - seg2.start.y)) + (seg2.stepsToEnd - (seg2.end.x - seg1.start.x))
		}
	} else {
		segmentsIntersect = ((seg1.start.x <= seg2.start.x && seg1.end.x >= seg2.start.x) || (seg1.end.x <= seg2.start.x && seg1.start.x >= seg2.start.x)) &&
			((seg2.start.y <= seg1.start.y && seg2.end.y >= seg1.start.y) || (seg2.end.y <= seg1.start.y && seg2.start.y >= seg1.start.y))
		if segmentsIntersect {
			intersection.intersect.x = seg2.start.x
			intersection.intersect.y = seg1.start.y
			intersection.stepsToIntersect = (seg1.stepsToEnd - int(math.Abs(float64(seg1.end.x-seg2.start.x)))) + (seg2.stepsToEnd - int(math.Abs(float64(seg2.end.y-seg1.start.y))))
		}
	}

	// if segmentsIntersect {
	// 	fmt.Printf("%v %v intersects at %v\n", seg1, seg2, intersection)
	// }

	return segmentsIntersect, intersection
}

func parseSegment(last segment, segmentStr string) segment {
	seg := segment{}
	seg.start = last.end

	dist, err := strconv.Atoi(segmentStr[1:])
	if err != nil {
		fmt.Printf("%s\n", err)
		os.Exit(1)
	}

	seg.stepsToEnd = last.stepsToEnd + dist

	switch segmentStr[0] {
	case 'U':
		seg.end = point{last.end.x, last.end.y + dist}
	case 'D':
		seg.end = point{last.end.x, last.end.y - dist}
	case 'L':
		seg.end = point{last.end.x - dist, last.end.y}
	case 'R':
		seg.end = point{last.end.x + dist, last.end.y}
	}
