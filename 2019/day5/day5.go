...
func intcode(input int) {
	...
	for i := 0; prog[i] != 99; i += opSize {
		...
		for j := 0; j < numParams; j, modes = j+1, modes/10 {
			if allowImmediateMode && modes%10 == 0 {
				params[j] = prog[prog[i+j+1]]
				fmt.Printf("[%d]=%d ", prog[i+j+1], params[j])
			} else {
				params[j] = prog[i+j+1]
				fmt.Printf("%d ", params[j])
			}
		}
